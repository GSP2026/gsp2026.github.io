---
layout: page
permalink: /registration/
title: "Registration"
description: 
nav: true
nav_order: 4

top_image: /assets/img/banners/banner_oso_GSP2.png
top_image_alt: "Madrid, Spain"
---

### Registration

Registration for GSP 2026 is now open. You can complete your registration [here](https://gac.afidcongresos.com/login_event.php?event=221). The registration fees are listed below.

<div style="margin: 1.5rem 0; padding: 1rem 1.25rem; border-left: 4px solid var(--global-theme-color); background: rgba(0, 0, 0, 0.03);">
  <strong>If you experience any problems during the registration process, please contact:</strong><br>
  <a href="mailto:info@afidcongresos.com">info@afidcongresos.com</a>
</div>

Registration fees:
+ Student - 420 &euro; (Early Bird, until April 23) / 520 &euro; (Regular)
+ Non-Student - 520 &euro; (Early Bird, until April 23) / 620 &euro; (Regular)
+ Gold Attendee - 1.200 &euro; (Early Bird, until April 23) / 1.200 &euro; (Regular)

The registration fee includes lunches and access to the social events.

Please note that each accepted paper must be covered by a registration, and each registration covers one paper. In exceptional cases, if you need to cover two papers with a single registration, please contact us at <a href="mailto:gspworkshop2026@gmail.com">gspworkshop2026@gmail.com</a>.

Early Bird fees are available until April 23, with regular fees applied from April 24.
